...
- Mercury Editor provides a cleaner, more focused in-context editing experience that is similar to Layout Builder. However sidebar UI
...
All of that is great, but their provider directory alone accounts for over 2,600 profiles. Not only that, its content is primarily — though not  — populated from an external Provider Data Management System (PDMS). This runs hourly and through the Migrate module.

The challenge then became, how do you provide the new page building features for these dynamically-fed types of content? Well, you prefill paragraph instances on all nodes based on contextual field data. For instance, each provider needed a listing of locations they were available at (adjacent to an interactive Google Map). Making editors hand-place that for 2,600 was not an option. So, I developed a script to add a locations listing map to the page building layout automatically — that ran immediately after the PDMS process completed.
